    ...
    <div class="d-md-none" id="collapsable-card" style="width: 80%">
      ...
      <div class="card" v-for="stock in stocks" v-bind:key="stock">
        <div class="card-header" :id="'heading' + stock.name">
          <button class="btn btn-link collapsed" data-bs-toggle="collapse" :data-bs-target="'#collapse'
+ stock.pk"
                  aria-expanded="true" :aria-controls="'collapse' + stock.pk">
            <h6 style="color: #0275d8; float: left">{{ stock.name }}</h6>
          </button>
        </div>
        <div :id="'collapse' + stock.pk" class="collapse" :aria-labelledby="'heading' +
stock.pk"
             data-bs-parent="#collapsable-card">
          <div class="card-body">
            <p><b>Customer:</b> {{ stock.customer_ }}</p>
            <p><b>Category:</b> {{ stock.symbol }}</p>
            <p><b>Description:</b> {{ stock.name }}</p>
            <div class="btn-group">
              <button @click="updateStock(stock)" style="background-color: transparent; padding: 5px;">
                <font-awesome-icon icon="pencil"/>
              </button>
              <button @click="deleteStock(stock)" style="background-color: transparent; padding: 5px;">
    ...
    <div class="col col-12 col-md-10 d-none d-xl-block d-lg-block d-md-block">
      ...
      <button type="button" class="btn btn-primary" @click="addNewStock">Add New
        Stock
      </button>
    </div>
  </div>
